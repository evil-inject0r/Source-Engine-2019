...
static LRESULT CALLBACK WndProc (HWND hwnd, UINT uMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KILLFOCUS:
	{
		mxWindow *window = (mxWindow *) GetWindowLong (hwnd, GWLP_USERDATA);
		if ( window )
		{
			mxEvent event;
	...
	case WM_COMMAND:
	{
		...
		if (LOWORD (wParam) > 0 && window)
		{
			...
			event.widget = (mxWidget *) GetWindowLong ((HWND) lParam, GWLP_USERDATA);
	...
	case WM_NOTIFY:
	{
		...
		if (nmhdr->code == TVN_SELCHANGED)
		{
			if (nmhdr->idFrom > 0)
			{
				mxWindow *window = (mxWindow *) GetWindowLong (hwnd, GWLP_USERDATA);
				event.event = mxEvent::Action;
				event.widget = (mxWidget *) GetWindowLong (nmhdr->hwndFrom, GWLP_USERDATA);
	...
	case WM_VSCROLL:
	{
		mxWidget *widget = (mxWidget *) GetWindowLong ((HWND) lParam, GWLP_USERDATA);
		if (!widget)
		{
			break;
		}
